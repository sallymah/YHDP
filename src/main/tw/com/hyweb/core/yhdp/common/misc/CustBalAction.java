 ...
 * author:anny
 */
public class CustBalAction
{
    protected TbCustBalInfo info = null;
    private Connection conn;
    private String m_sTable = "TB_CUST_BAL";
    private static Logger log = Logger.getLogger(CustBalAction.class);

    public CustBalAction(Connection conn, TbCustBalInfo info)
    {
        this.conn = conn;
        this.info = info;
    }
    ...
    public TbCustBalInfo getCardBalInfo()
    {
        return info;
    }

    public void setCardBalInfo(TbCustBalInfo cardBalInfo)
    {
        if (cardBalInfo != null) {
            this.info = (TbCustBalInfo) cardBalInfo.clone();
        }
    }
    ...
    protected String getCond() throws NullPointerException {
        ...
        if (!StringUtil.isEmpty(info.getCustId())) {
          sql.append(iCond > 0 ? " AND " : " WHERE ");
          sql.append("CUST_ID = ");
          sql.append(StringUtil.toSqlValueWithSQuote(info.getCustId()));
          iCond++;
        }
