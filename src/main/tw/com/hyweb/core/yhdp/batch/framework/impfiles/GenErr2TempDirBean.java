...
import tw.com.hyweb.core.yhdp.batch.framework.filesin.FilesIn;
...
public class GenErr2TempDirBean {
    ...
    protected void genErr2TempDir(String where, String fullName) throws Exception {
    	...
    	if (fileInfo.getSubFileName().contains("/")){
    		...
    		fileOk = fullName.substring(0,fullName.lastIndexOf(".")) + subFileName;
    	}
    	else{
    		subFileName = fileInfo.getSubFileName();
    		fileOk = f.getAbsolutePath() + subFileName;
    	}
        
        File okf = new File(fileOk);
