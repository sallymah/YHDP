...
public class ImpFileInfo {
    ...
    public boolean checkFile2715() throws Exception {
    	...
    	if (mappingInfo.getHeader().isHasHeader() == false) {
        	...
        	{
        		info = result.get(0);
        		if (info.getPosSimpleCode() == null 
        				|| "".equals(info.getPosSimpleCode().trim())) 
                    isCheck = false;
        	} else {
        		isCheck = false;
        	}
        	...
        	if (groupCnt > 0 && groupResult.size() > 0) 
        	{
        		...
        		if (groupInfo.getPosSimpleCode() == null 
        				|| "".equals(groupInfo.getPosSimpleCode().trim()))
        			isCheck = false;
        	} else {
        		isCheck = false;
        	}
