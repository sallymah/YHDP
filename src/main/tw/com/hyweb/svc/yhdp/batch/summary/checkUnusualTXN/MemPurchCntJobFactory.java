...
public class MemPurchCntJobFactory extends CursorBatchJobFactory {
	private int seqNo = 0;
	/*
     * (non-Javadoc)
     * 
    ...
    {
    	String sql = "SELECT TB_TRANS.ACQ_MEM_ID,CARD_NO, COUNT(*), MAX_TXN_COUNT FROM TB_TRANS,TB_MEMBER " +
    			"WHERE TB_TRANS.ACQ_MEM_ID = TB_MEMBER.MEM_ID " +
    			"AND CUT_DATE = '"+batchDate+"' " +
    			"AND P_CODE = '7647' " +
    			"AND MAX_TXN_COUNT <> 0 " +
    			"GROUP BY TB_TRANS.ACQ_MEM_ID,CARD_NO, MAX_TXN_COUNT " +
    			"HAVING COUNT(*) > MAX_TXN_COUNT ";
    ...
    {
    	seqNo++;
        return new MemPurchCntJob(result,seqNo);
    }
}
