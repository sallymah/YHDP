...
public class RetBoundData
{
    private static Logger log = Logger.getLogger(RetBoundData.class);
    ...
    public RetBoundData(Connection connection, Map<String, String> fileData) throws SQLException
    {
    	this.fileData = fileData;
    	this.cardNo = fileData.get("CARD_NO");
    	this.count = getCardInfo(connection , cardNo);    
    }
    ...
    public List handleRetBound(Connection connection, String batchDate) throws Exception {
    	
    	List sqls = new ArrayList();
    	
    	String updateCardSql = makeReturnBound(batchDate);
    ...
    private String makeReturnBound(String batchDate) {
    	
    	String lifeCycle="1";  	
    	String revLifeCycle = result.get(0).getLifeCycle();
    	String revStatus = result.get(0).getStatus();
    	//String sql = "UPDATE TB_CARD SET LIFE_CYCLE='" + lifeCycle + "' WHERE CARD_NO='" + cardNo + "'";
    	String sql = "UPDATE TB_CARD SET LIFE_CYCLE='" + lifeCycle 
    			+ "', PREVIOUS_LIFE_CYCLE='" + revLifeCycle 
    			+ "', PREVIOUS_STATUS='" + revStatus 
    			+ "', CUSTOMER_ID='" + fileData.get("CUSTOMER_ID") 
    			+ "', SALE_WAY='" + fileData.get("SALE_WAY") 
    			+ "', UPT_DATE='" + sysDate 
    			+ "', UPT_TIME='" + sysTime 
    			+ "', UPT_USERID='batch', APRV_DATE='" + sysDate 
    			+ "', APRV_TIME='" + sysTime 
    			+ "', APRV_USERID='batch', UPDATE_TYPE='2' WHERE CARD_NO='" + cardNo + "'";
