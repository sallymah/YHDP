...
public class MemReloadAmtJobFactory extends CursorBatchJobFactory {
	private int seqNo = 0;
	/*
     * (non-Javadoc)
     * 
    ...
    {
    	String sql = "SELECT CARD_NO, SUM(TXN_AMT) ,MAX_RELOAD_AMT FROM TB_TRANS,TB_MEMBER "+
    			"WHERE TB_TRANS.ACQ_MEM_ID = TB_MEMBER.MEM_ID "+
    			"AND CUT_DATE = '"+batchDate+"' "+
    			"AND P_CODE = '7707' " +
    			"AND MAX_RELOAD_AMT <> 0 "+
    			"GROUP BY TB_TRANS.ACQ_MEM_ID,CARD_NO, MAX_RELOAD_AMT "+
    			"HAVING SUM(TXN_AMT) > MAX_RELOAD_AMT ";
    ...
    {
    	seqNo++;
        return new MemReloadAmtJob(result,seqNo);
    }
}
