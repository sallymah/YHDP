        ...
        try {
	        ...
	        for (int i = 0; i < inctlInfos.size(); i++) {
	            try {
	                ...
	                impFileInfo.setCheckOKFile(checkOKFile);
	                impFileInfo.setCheckAcqMemId(checkAcqMemId);
	                //客製化encoding 撈取TB_MEMBER
	                if (mappingInfo.isHasCustEncoding()){
	                	String encoding;
	                	String memEncoding = DbUtil.getString("SELECT FILE_FORMAT FROM TB_MEMBER WHERE MEM_ID = " + StringUtil.toSqlValueWithSQuote(inctlInfo.getMemId()), connection);
	                	log.debug(inctlInfo.getMemId() + " memEncoding: " + memEncoding);
	                	//TB_MEMBER未設定則使用MappingInfo設定
	                	if (StringUtil.isEmpty(memEncoding)){
	                		encoding = mappingInfo.getEncoding();
	                	}
	                	//TB_MEMBER為主，如設定為為定義編碼，則以MappingInfo設定為主
	                	else{
	                		Object mlEncoding = ml.getEncodingInfos().get(memEncoding);
	                		encoding = mlEncoding == null ? mappingInfo.getEncoding() : mlEncoding.toString();
	                	}
	                	impFileInfo.setEncoding(encoding);
	                	log.debug(inctlInfo.getMemId() + " encoding: " + encoding);
	                }
	                else{
	                	impFileInfo.setEncoding(mappingInfo.getEncoding());
	                }
	                impFileInfo.setCheckEmptyFile(checkEmptyFile);
	                // handle impFileInfo
	                handleImpFileInfo();
            ...
            thisRcode = Layer1Constants.RCODE_2999_ALLDATAERROR;
            log.error("handleImpFileInfo error:" + ignore.getMessage(), ignore);
        }
        finally {
