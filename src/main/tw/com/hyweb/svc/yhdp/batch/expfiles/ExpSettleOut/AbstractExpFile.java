...
public abstract class AbstractExpFile extends AbstractBatchBasic {
	...
	private boolean settleOutFlag = true;
	...
	public boolean isSettleOutFlag() {
		return settleOutFlag;
	}

	public void setSettleOutFlag(boolean settleOutFlag) {
		this.settleOutFlag = settleOutFlag;
	}
	...
	public abstract String outputOneRecord(List record);
...
//        sql.append(" AND SYS_DATE = " + StringUtil.toSqlValueWithSQuote(batchDate));
    ...
    public void process(String[] args) throws Exception {
    	...
    	ArrangeSettleOut arrangeSettleOut = new ArrangeSettleOut(getBatchResultInfo());
    	
    	try{
	    	if (settleOutFlag){
	    		// 先清除 TB_TB_SETTLEOUT 資料
	    		arrangeSettleOut.recoverData(batchDate);
	    		// 先處理 TB_TB_SETTLEOUT 資料
		    	arrangeSettleOut.action(batchDate);	
	    	}
	        ...
	        for (int i = 0; i < expFileSetting.getExpFileInfos().size(); i++) {
	            try {
	                ...
	                // 遠鑫需求 SettleOut 不匯出空檔
	                if (expFileResult.getTotalRecords() > 0){
		                handleOneExpFile();
		                Thread.sleep(sleepPerInfo);
	                }
	                else{
	                	log.warn(expFileResult.getExpFileInfo().getMemId() + " TotalRecords [" + expFileResult.getTotalRecords() + "]");
	                }
	            }
	            catch (Exception ignore) {
    		...
    		//失敗recover TB_TB_SETTLEOUT 資料
    		log.warn("recoverData");
    		throw ignore;
    	}
    	finally {
            ReleaseResource.releaseDB(conn);
        ...
        setRcode(thisRcode);
        if (errorDesc == null)
        	errorDesc = "success Exp Files:" + expFileSetting.getExpFileInfos().size();
