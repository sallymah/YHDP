...
public class FF21
{
	...
	public FF21(int len , String str){
...
/*			    if(respCode.equals("80E1"))
			...
			autoLoadAmt = str.substring(idx, idx+=10);
			autoLoadAtc = str.substring(idx, idx+=8);
		}
	}
	
	public String pack(){
		return ISOUtil.hexString(respCode.getBytes())+ISOUtil.hexString(tid.getBytes())+ISOUtil.hexString(mid.getBytes())+batchNo+issNo+ISOUtil.hexString(storeCntArea.getBytes())+iccAtc+termDateTime+hostDateTime+ecashAmt+lmsAmt+samArea+ISOUtil.hexString(chipCardSts.getBytes())+ISOUtil.hexString(accessMod.getBytes())+ecashBeforeBal+autoLoadAmt+autoLoadAtc;
	}
   ...
   public String getAutoLoadAmt() {
	...
	public String toString() {
		return "FF21 [\nrespCode: "+respCode
				...
				+"\nlmsAmt: "+lmsAmt
				+"\nsamArea: "+samArea
				+"\nchipCardSts: "+chipCardSts
				+"\naccessMod: "+accessMod
				+"\necashBeforeBal: "+ecashBeforeBal
				+"\nautoLoadAmt: "+autoLoadAmt
				+"\nautoLoadAtc: "+autoLoadAtc+"]";
	}
