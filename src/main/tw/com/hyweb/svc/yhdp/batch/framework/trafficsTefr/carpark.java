...
public class carpark extends FilenameBean{
	...
	public void initial(String fullFileName, HashMap members) throws Exception {
		...
		String memId = AfterSplit[1];
		String dateSeqno = AfterSplit[2];
		//20161024 遠鑫提出修改 流水號改為3碼
		String seqno = dateSeqno.substring(dateSeqno.length()-3, dateSeqno.length());
		String unzipDateTime = AfterSplit[3];
		
		TbMemberInfo tbMemberInfo = (TbMemberInfo) members.get(memId);
		...
		if ( tbMemberInfo != null ){
			...
			setFullFileNameR(TEFR_FN + "_" + memId + "_" + dateSeqno + "_" + unzipDateTime );
		}
	}
