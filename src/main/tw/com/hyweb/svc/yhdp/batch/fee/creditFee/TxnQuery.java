...
public class TxnQuery {
	...
	protected ProcCreditFee procCreditFee;
    ...
    protected TxnQuery(ProcCreditFee procCreditFee)
	{
		this.procCreditFee = procCreditFee;
	}
     ...
     * @param pCodeList
     */
    protected void setTxnCondition( CreditFeeConfig creditFeeConfig )
    {
        ...
        if (creditFeeConfig.getFeeTarget().equals("1")){
        	...
        	sb.append(" WHERE BANK_ID = '"+creditFeeConfig.getBankId()+ "' ");
        	sb.append(" AND SALE_CODE = '"+creditFeeConfig.getSaleCode()+ "' ");
        	
        	if ( creditFeeConfig.getCalRangeFlag() != null && creditFeeConfig.getCalRangeFlag().equals("1") ){
        		sb.append("AND AUTO_RELOAD_FLAG='Y' ");
        	}
        	
        	sb.append(") ");
        }
        else{
	        if (!creditFeeConfig.getAcqMemId().equals(procCreditFee.getAllMember()) && creditFeeConfig.getFeeTarget().equals("0")){
	        	sb.append("AND ACQ_MEM_ID = '").append(creditFeeConfig.getAcqMemId()).append("' ");
	        }
	        if ( creditFeeConfig.getCalRangeFlag() != null && creditFeeConfig.getCalRangeFlag().equals("1") ){
	        	sb.append("AND EXISTS (SELECT CARD_NO FROM TB_CARD WHERE AUTO_RELOAD_FLAG='Y' AND TB_CREDIT_TXN.CARD_NO = TB_CARD.CARD_NO) ");
	        }
        }
    ...
    protected ProcCreditFee getProcFee()
    {
		return procCreditFee;
	}

    protected void setProcFee(ProcCreditFee procCreditFee) 
    {
		this.procCreditFee = procCreditFee;
	}
