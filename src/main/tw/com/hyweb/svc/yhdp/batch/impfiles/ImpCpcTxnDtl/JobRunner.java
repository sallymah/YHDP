...
public class JobRunner extends AbstractImpFile implements Runnable
{
    ...
    private final static String HEX_DETAIL_KEY = "443031"; //D01
    ...
    public List handleDataLine(DataLineInfo lineInfo) throws Exception
    {
        ...
        if(lineStr.substring(0, HEX_DETAIL_KEY.length()).equals(HEX_DETAIL_KEY))//去除非 D01以外欄位
        {
            parserStr = lineStr.substring(HEX_DETAIL_KEY.length());
        }
        ctx.setRawData(parserStr);//去除ff10再取raw data
        ...
        if(berTLV.hasTag(0xFF10))
        {
            ...
            ctx.setRawData(ISOUtil.hexString(berTLV.pack()));//去除ff10再取raw data
        }
         ...
         // beforeHandleDataLine
            ...
            for (int i = start; i < totRec; i++) {
                ...
                try {
                    ...
                    if(lineInfoStr.substring(0, HEX_DETAIL_KEY.length()).equals(HEX_DETAIL_KEY))//去除非 D01以外欄位
                    {
                        String parserStr = lineInfoStr.substring(HEX_DETAIL_KEY.length());
    ...
    public void InsertTermBatchCpcProc(BATCHContext ctx) throws Exception
    {
        ...
        if(inctlInfo.getFileName().equals("IMPCPCTXNDTL"))//中油
        {
            betchNo = tlv.getHexStr(LMSTag.BatchNumber);
            isCpc = 1;
        }
        
        if(inctlInfo.getFileName().equals("IMPFMTXNDTL"))//全家
        {
            betchNo = getMaxBatchNo(ctx, merchId, termId);//全家沒批號可能無法結帳online交易,需再確認
            isCpc = 2;
        }
    ...
    public void insertTmpTrans(FF21 ff21, DataLineInfo lineInfo, TbInctlInfo inctlInfo, String rcode, String rawData) throws SQLException
    {
        ...
        String plainLin = lineInfo.getPlainLine().substring(HEX_DETAIL_KEY.length());
        String ff10Sample = "FF100131";
        String ff10Str = plainLin.substring(plainLin.length() - ff10Sample.length(), plainLin.length() - 2);
        if(ff10Str.equals("FF1001"))
        {
            tmpTransInfo.setLineData(plainLin.substring(0, plainLin.length() - ff10Sample.length()));
        }
        else
        {
            tmpTransInfo.setLineData(lineInfo.getPlainLine().substring(HEX_DETAIL_KEY.length()));
        }
        ...
        if(!rcode.equals("0000"))
        {
            ...
            {
                if(ff21.respCode.equalsIgnoreCase(rcode))//當交易為失敗的交易,檢查是否因為原交易失敗而踢錯,如果是則回覆處理成功
                {
                    rcode = "0000";
                }
            }
