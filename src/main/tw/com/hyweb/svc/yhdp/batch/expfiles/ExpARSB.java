 ...
 * author:duncan
 */
public class ExpARSB extends AbstractExpFile {
    private static Logger log = Logger.getLogger(ExpARSB.class);
    private static final String SPRING_PATH = "config" + File.separator + "batch" + File.separator +
            "expfiles" + File.separator + "ExpARSB" + File.separator + "spring.xml";
    private static final String FILE_FN = "ARSB";
    private static final String FILE_APPEND = "ARSB";
    ...
    private void beforeHandling() throws Exception 
    {
		...
		try{
			...
			sqlCmd.append("select MEM_ID, BANK_ID, MEM_GROUP_ID from TB_MEMBER WHERE SUBSTR(MEM_TYPE,3,1)='1'");
			Vector vr = DbUtil.select(sqlCmd.toString(), conn);
    ...
    public ExpFileSetting makeExpFileSetting() {
        ...
        if(bankIds == null){
        	log.error("they are no any bank id !!");
        	return null;
        }
        for (int i = 0; i < bankIds.size(); i++) {
            ...
            sb.append("Select CARD_NO, EXPIRY_DATE, TXN_DATE, TXN_TIME, TXN_AMT,");
            sb.append(" ACQ_MEM_ID, MERCH_ID, LMS_INVOICE_NO, P_CODE, TXN_SRC from TB_TRANS");
            sb.append(" Where CUT_DATE='").append(batchDate).append("'");
            sb.append(" and P_CODE IN ('7717','7737','7438')");
            sb.append(" and CARD_NO in (select CARD_NO from TB_CARD where BANK_ID='").append(bankId).append("')");
            sb.append(" union");
            sb.append(" Select CARD_NO, EXPIRY_DATE, TXN_DATE, TXN_TIME, TXN_AMT,");
            sb.append(" ACQ_MEM_ID, MERCH_ID, LMS_INVOICE_NO, ");
            sb.append(" CASE P_CODE WHEN '5717' THEN '4717' WHEN '5737' THEN '4737' END as P_CODE,");
            sb.append(" 'R' from TB_AUTO_RELOAD_RESP");
            sb.append(" Where STATUS='").append("1").append("'");
            sb.append(" and P_CODE in ('5717','5737') ");
            sb.append(" and EXP_DATE='").append("00000000").append("'");
            sb.append(" and CARD_NO in (select CARD_NO from TB_CARD where BANK_ID='").append(bankId).append("')");
    ...
    public String outputBeforeFile() {    	
        ...
        header.append(StringUtils.rightPad("", 108, ' '));
    ...
    { 
    	...
    	if(memInfo == null) throw new Exception("acquirer member id("+ acqMemId +") is not exist !!");
    	TbMerchInfo merchInfo = new TbMerchMgr(conn).querySingle(merchId);
    	if(merchInfo == null) throw new Exception("merchant id("+ merchId +") is not exist !!");
...
//        sb.append(Layer2Util.getMaxString(StringUtils.rightPad(merchInfo.getMerchLocName(), 20, ' '),encoding,20));
        ...
        else if (txnSrc.equals("S")) {
        	sb.append(StringUtils.rightPad("1", 1, ' '));
        }
        else {
        	sb.append(StringUtils.rightPad("2", 1, ' '));
        }
        //sb.append(StringUtils.rightPad(txnSrc, 1, ' '));
	...
	{
		if(dataInfo.getpCode().equals("4717")) 
        {
			BankUtil.simulateTrans(conn, dataInfo, batchDate, "4717", "8717");
        }
		if(dataInfo.getpCode().equals("4737")) 
        {
			BankUtil.simulateTrans(conn, dataInfo, batchDate, "4737", "8717");
        }
        modifyTrans(dataInfo);
        
        if(dataInfo.getTxnSrc().equals("R")) {
	...
	private void modifyAutoReloadResp(ExpDataInfo dataInfo) throws Exception 
	{
		...
		sb.append(" ,STATUS='").append("2").append("'");
	...
	public static ExpARSB getInstance() {
        ApplicationContext apContext = new FileSystemXmlApplicationContext(SPRING_PATH);
        ExpARSB instance = (ExpARSB) apContext.getBean("expARSB");
        return instance;
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            log.warn("ExpARSB run fail:" + ignore.getMessage(), ignore);
        }
    }
}
