...
public abstract class AbstractExpFile extends AbstractBatchBasic {
    ...
    public abstract byte[] outputOneRecord(List record) throws Exception;
    
    public abstract List outputDtlRecord(List record);
    
    public abstract byte[] outputDecryptBeforeFile( ) throws Exception;
    ...
    // 加密
    public void decryptTempFile() throws Exception {
    	
    	try {
	    	byte[] macHeaderRecord = outputDecryptBeforeFile();
			
	    	String file = expFileInfo.getExpTempFile().toString();
			BufferedInputStream br = new BufferedInputStream(new FileInputStream(file));
			BufferedOutputStream ps = new BufferedOutputStream(new FileOutputStream(new File(file+".XOR")));
			int count = 0;
	        ...
	        while(br.available()>0){
	        	...
	        	/*if (bytesRead==0x0D && count == 0){
	        		count++;
	        		ps.write(macHeaderRecord);
	        		ps.write(0x0D);
	        	}*/
	        	//20151224 因交通黑名單改為無換行符號，原本加密後換第一行，改為取代固定長度
	        	if (count < macHeaderRecord.length){
	        		
	        		ps.write(macHeaderRecord[count]);
	        		count++;
			...
			File f2 = new File(file+".XOR");
			f2.renameTo(expFileInfo.getExpTempFile());
    	}
    	catch (Exception ignore) {
    		throw new Exception("decryptTempFile Error! " + ignore);
    	}
    }
    ...
    public void closeFileProcess() throws Exception {
        ...
        try {
        	byte[] outputString = outputAfterFile();
            if (outputString.length > 0) {
                bw.write(outputString);
                bw.write(expFileInfo.getLineSeparator().getBytes());
            }
            bw.flush();
            bw.close();
...
//                expFileInfo.getExpTempFile().delete();
    ...
    public void handleOneExpFile() throws Exception {
        ...
        try {
            ...
            if (expFileInfo.isUsingTempFile()) {
            	bw = new BufferedOutputStream(new FileOutputStream(expFileInfo.getExpTempFile()));
            }
            else {
            	bw = new BufferedOutputStream(new FileOutputStream(expFileInfo.getExpFile()));
            }
            // 處理 outputBeforeFile 動作
            byte[] outputString = outputBeforeFile();
            if (outputString.length > 0) {
                bw.write(outputString);
                bw.write(expFileInfo.getLineSeparator().getBytes());
            }
