...
public abstract class AbstractExpFile extends AbstractBatchBasic {
    ...
    public void locateFile() throws Exception {
        ...
        final String HG_CARD_GROUP ="HG_CARD_GROUP";
        String workDir = "";
        String relativePath = "";
        try {
        ...
        if (expFileInfo.getFileInfo().getLocalPath().indexOf(HG_CARD_GROUP) != -1) {
            // find "HG_CARD_GROUP"
            relativePath = relativePath.replaceAll(HG_CARD_GROUP, expFileInfo.getMemId());
        }
...
//                expFileInfo.getExpTempFile().delete();
    ...
    public void handleOneExpFile() throws Exception {
        ...
        try {
        	...
        	//失敗Rollback 前面做的 TB_CARD_BAL、TB_TRANS、TB_TRANS_DTL、TB_TERM_BATCH
        	rollback();
