...
public class ImpFileInfo {
    ...
    private boolean checkAcqMemId = true;
    ...
    public boolean isCheckAcqMemId() {
		return checkAcqMemId;
	}

	public void setCheckAcqMemId(boolean checkAcqMemId) {
		this.checkAcqMemId = checkAcqMemId;
	}
    ...
    public void computeTotRecByReadFile() {
        ...
        try {
            tmpbr = new BufferedReader(new InputStreamReader(new FileInputStream(file), encoding));
    ...
    public boolean checkFile2702() throws Exception {
        if (StringUtil.isEmpty(mappingInfo.getSeparatorString())) {
        	log.info("inctl file_size"+inctlInfo.getFileSize().intValue());
        	log.info("fileInfo datalen"+fileInfo.getDataLen().intValue());
        ...
        if (StringUtil.isEmpty(mappingInfo.getSeparatorString())) {
            try {
                for (int i = 0; i < mappingInfo.getHeader().getNumLines(); i++) {
                    ...
                    lines.add(new String(buff, 0, readLen, encoding));
                }
            }
            catch (Exception ignore) {
    ...
    public boolean checkFile2703() throws Exception {
        if (mappingInfo.getHeader().isHasHeader() == false) {
            ...
            if (StringUtil.isEmpty(mappingInfo.getSeparatorString())) {
                ...
                String headerTag = new String(buff, 0, 2, encoding);
    ...
    public boolean checkFile2704() throws Exception {
        if (mappingInfo.getHeader().isHasHeader() == false) {
            ...
            if (StringUtil.isEmpty(mappingInfo.getSeparatorString())) {
                ...
                final String HEADER_TAG = "H0";
                String headerTag1 = new String(buff1, 0, 2, encoding);
                String headerTag2 = new String(buff2, 0, 2, encoding);
    ...
    public boolean checkFile2706() throws Exception {
        if (mappingInfo.getHeader().isHasHeader() == false) {
            ...
            if (StringUtil.isEmpty(mappingInfo.getSeparatorString())) {
                ...
                int hnumLines = -1;
                try {
                    String str = new String(buff, 10, 8, encoding);
    ...
    public boolean checkFile2707() throws Exception {
        if (mappingInfo.getHeader().isHasHeader() == false) {
            ...
            if (StringUtil.isEmpty(mappingInfo.getSeparatorString())) {
                ...
                // see filespec
                String hfileDate = new String(buff, 2, 8, encoding);
    ...
    public boolean checkFile2715() throws Exception {
    	...
    	if (mappingInfo.getHeader().isHasHeader() == false) {
            ...
            if (StringUtil.isEmpty(mappingInfo.getSeparatorString())) {
                ...
                // see filespec
                String hacqMemId = new String(buff, 18, 8, encoding);
                ...
                if (acqMemId.equals(hacqMemId)) {
                    ...
                    rCode = Layer1Constants.RCODE_2715_ACQMENID_ERR;
                    return false;
                }
            }
            else {
        	...
        	String memId = inctlInfo.getMemId();
        	Connection conn = DBService.getDBService().getConnection(Layer1Constants.DSNAME_BATCH);
        	...
        	{
        		info = result.get(0);
        		if (info.getSimpleCode() == null 
        				|| "".equals(info.getSimpleCode().trim())) 
                    isCheck = false;
        	} else {
        		isCheck = false;
        	}
        	
        	Vector<TbMemberGroupInfo> groupResult = new Vector<TbMemberGroupInfo>();
        	TbMemberGroupInfo groupInfo = new TbMemberGroupInfo();
        	groupInfo.setMemGroupId(info.getMemGroupId());
        	int groupCnt = new TbMemberGroupMgr(conn).queryMultiple(groupInfo, groupResult);
        	if (groupCnt > 0 && groupResult.size() > 0) 
        	{
        		groupInfo = groupResult.get(0);
        		if (groupInfo.getSimpleCode() == null 
        				|| "".equals(groupInfo.getSimpleCode().trim()))
        			isCheck = false;
        	} else {
        		isCheck = false;
        	}
        	...
        	{
                FieldInfo fieldInfo = new FieldInfo();
                fieldInfo.setDesc("企業代碼");
                insertInctErr(Layer1Constants.RCODE_2715_ACQMENID_ERR, fieldInfo, memId);
                rCode = Layer1Constants.RCODE_2715_ACQMENID_ERR;
        	}
        	ReleaseResource.releaseDB(conn, null, null);
            return isCheck;
                ...
                try {
                    ...
                    for (int i = 0; i < mappingInfo.getFields().size(); i++) {
                        ...
                        if (FieldInfo.TYPE_STRING.equals(fieldInfo.getType())) {
                            // type is string
                            String data = new String(buff, fieldInfo.getStart(), fieldInfo.getLength(), encoding);
