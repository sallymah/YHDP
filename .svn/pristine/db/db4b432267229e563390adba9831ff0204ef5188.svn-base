...
public abstract class AbstractImpFile extends AbstractBatchBasic {
    ...
    protected MappingInfo mappingInfo = null;//RECORD INFORMATION
    ...
    protected int recordsPerCommit = 0;
    // millisecond
    protected int sleepPerCommit = 0;
    ...
    public void insertInctlErrInfo(DataLineInfo lineInfo, List descInfos) throws Exception {
        impFileInfo.setFailCnt(impFileInfo.getFailCnt() + 1);
        TbInctlErrInfo inctlErrInfo = null;
        try {
            inctlErrInfo = ImpFilesUtil.makeInctlErrInfo(inctlInfo, lineInfo, descInfos);
            TbInctlErrMgr mgr = new TbInctlErrMgr(conn);
            mgr.insert(inctlErrInfo);
        }
        catch (Exception ignore) {
            log.error(inctlErrInfo.toInsertSQL());
            log.error("insertInctlErrInfo error:" + ignore.getMessage(), ignore);
            throw ignore;
        }
    }

    public void insertInctlErrInfo(DataLineInfo lineInfo, ErrorDescInfo descInfo) throws Exception {
        List descInfos = new ArrayList();
        descInfos.add(descInfo);
    ...
    public boolean handleImpFileInfo() throws Exception {
        ...
        try {
            ...
            // checkFile
            boolean ret = impFileInfo.checkFile(ImpFileInfo.PARSER_BYTE);
            if (!ret) {
            ...
            for (int i = start; i < impFileInfo.getTotRec(); i++) {
                ...
                try {
                    ...
                    descInfos = checkDataLine(lineInfo);                    
    ...
    public void process(String[] args) throws Exception {
        ...
        // fileInfo check
        fileInfo = ImpFilesUtil.getFileInfoIn(fileName);//TB_IFLE_INFO.file_name
        // no fileInfo
        if (fileInfo == null) {
