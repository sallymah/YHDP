...
{
    private static Logger log = Logger.getLogger(InBoundData.class);
    ...
    public InBoundData(Connection connection, Map<String, String> fileData) throws SQLException
    {
    ...
    public List handleInBound(Connection connection, String batchDate) throws Exception {
    	...
    	String updateCardSql = makeInBound(batchDate);
	...
	private String makeInBound(String batchDate) 
	{
		String sql = "";
		String lifeCycle = "1";
		String revLifeCycle = result.get(0).getLifeCycle();
		String revStatus = result.get(0).getStatus();
		/*if(getCardInfo().getCardNo().substring(0, 3).equals("986")) {
			lifeCycle="2";  	
		}
		else {
			lifeCycle="1";  		
		...
		sql = "UPDATE TB_CARD SET LIFE_CYCLE='" + lifeCycle 
    	...
    	//String sql = "UPDATE TB_CARD SET LIFE_CYCLE='" + lifeCycle + "', CUSTOMER_ID='" + fileData.get("CUSTOMER_ID") + "', SALE_WAY='" + fileData.get("SALE_WAY") + "' WHERE CARD_NO='" + fileData.get("CARD_NO") + "'";
