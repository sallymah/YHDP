...
public class JobRunner implements Runnable
{
	...
	private TbMemberInfo memberInfo;
	private Object decryptionUtil;
	private List hms;
	private int idx;
     ...
     * @param lBody - raw data body
     */
    public JobRunner(HashMap<String,List> resultMap, int idx, TbMemberInfo memberInfo, Object decryptionUtil, List hms)
    {
    ...
    public void run()
    {
        ...
        try {
            ...
            List threadStringList = handleImpFileInfo();
            resultMap.put(key,threadStringList);
        }
        catch (Exception e) {
...
//        }
    ...
    public List handleImpFileInfo() throws Exception {

    	List threadStringList = new ArrayList<>();
    	...
    	try
        {
        	for( int i = 0; i < hms.size(); i++ ){
        		...
        		if (memberInfo != null){
					//銀行卡加密方式
        			CgYhdpSsf cgYhdpSsf = (CgYhdpSsf) decryptionUtil;
					enString = cgYhdpSsf.decryptDES3(ISOUtil.hexString(dtlBuffer));
				}
				else{
					HsmAdapterECB hsmAdapterECB = (HsmAdapterECB) decryptionUtil;
					enString = hsmAdapterECB.descryptECB(ISOUtil.hexString(dtlBuffer));
				}
        		
        		threadStringList.add(enString);
        	}
        }
        catch (Exception e)
        {
	...
	public void setResultMap(HashMap<String, List> resultMap) {
		this.resultMap = resultMap;
	}
