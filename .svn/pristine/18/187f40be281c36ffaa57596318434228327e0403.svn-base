...
public class SupplyJob {
	...
	private final TbRmcardBalSumInfo rmcardBalSumInfo;
	...
	public SupplyJob (TbRmcardBalSumInfo rmcardBalSumInfo, String batchDate, 
			InsertTermBatchHandle termBatchHandle, List<TbBonusIssDefInfo> tbBonusIssDefList) {
		this.rmcardBalSumInfo = rmcardBalSumInfo;
	...
	public void init(Connection conn) throws SQLException {
        ...
        pk.setCardNo(rmcardBalSumInfo.getCardNo());
        pk.setExpiryDate(rmcardBalSumInfo.getExpiryDate());
		...
		params.add(rmcardBalSumInfo.getCardNo());
		params.add(rmcardBalSumInfo.getExpiryDate());
	...
	public void appointAction(Connection conn) throws Exception{
		...
		params.add(rmcardBalSumInfo.getCardNo());
		params.add(rmcardBalSumInfo.getExpiryDate());
		params.add(rmcardBalSumInfo.getTxnDate());
		...
		rmcardBalSumInfo.setNotDwAptAmt(notDwAptAmt);
	}
	...
	public void trtsRewardAction(Connection conn) throws Exception{
		...
		/*計算上一個月北捷回饋金*/
		String lastSDate = (DateUtil.addMonth(rmcardBalSumInfo.getTxnDate(), -1)).substring(0, 6) + "01";
		...
		/*計算本月北捷回饋金*/
		String thisSDate = rmcardBalSumInfo.getTxnDate().substring(0, 6) + "01";
		String thisEDate = rmcardBalSumInfo.getTxnDate();
		...
		rmcardBalSumInfo.setMetroLastMonthRewardAmt(lastMonReward);
		rmcardBalSumInfo.setMetroThisMonthRewardAmt(thisMonReward);
	}
	...
	private List<TbCardBalInfo> makeCardBal(Connection conn) throws SQLException
    {
        ...
        cardBalSqlCmd.append(" CARD_NO = ").append(StringUtil.toSqlValueWithSQuote(rmcardBalSumInfo.getCardNo()));
        cardBalSqlCmd.append(" AND EXPIRY_DATE = ").append(StringUtil.toSqlValueWithSQuote(rmcardBalSumInfo.getExpiryDate()));
