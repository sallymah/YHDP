 ...
 * --------------------
 */
package tw.com.hyweb.svc.yhdp.batch.framework.impfiles.bank;
...
public abstract class AbstractImpFile extends AbstractBatchBasic {
    ...
    protected boolean checkAcqMemId = true;
    ...
    public boolean handleImpFileInfo() throws Exception {
        ...
        try {
            ...
            if (!ret) {
                // checkFile fail, 整檔踢退, 不處理，需單筆寫入TABLE
            	rCode = impFileInfo.getrCode();
            	impFileInfo.skipHeader();
            	inctlInfo.setRcode(Layer1Constants.RCODE_2999_ALLDATAERROR);
            	//impFileInfo.setFailCnt(impFileInfo.getFailCnt() + 1);
            ...
            log.info("getTotRec" + impFileInfo.getTotRec());
                ...
                try {
                    ...
                    if(rCode.equals("0000")) {
                        ...
                        if (descInfos != null && descInfos.size() > 0) {
                        	...
                        	rCode = ((ErrorDescInfo)descInfos.get(0)).getErrorCode();
                            if (doCommit) {
                            	log.info("insert inctl err(docommit)");
                                insertInctlErrInfo(lineInfo, descInfos);
                                //失敗註記
                                log.info("insert data log");
                                insertImportDataLogInfo(conn, lineInfo, rCode);
                                //initial rcode
                                rCode="0000";
                                continue;
                            }
                    ...
                    //成功註記
                    log.info("insert data log(success)");
                    insertImportDataLogInfo(conn, lineInfo, rCode);
                    ...
                    if (doCommit) {
                    	log.info("insert inctl err(exception docommit)");
                        insertInctlErrInfo(lineInfo, descInfo);
                        insertImportDataLogInfo(conn, lineInfo, descInfo.getErrorCode());
                    }
                    else {
                        // 有錯整個檔案都不處理
                        // 先做 rollback, 再 TB_INCTL_ERR 記錄那一筆出錯, 並跳開
                    	log.info("insert inctl err(exception)");
                        conn.rollback();
                        insertInctlErrInfo(lineInfo, descInfo);
                        break;
                    }
            ...
            // set rcode by xxxCnt
            if (impFileInfo.getTotRec() >0 && impFileInfo.getTotRec() == impFileInfo.getFailCnt()) {
                thisRcode = Layer1Constants.RCODE_2999_ALLDATAERROR;
            }
    ...
    public void process(String[] args) throws Exception {
        ...
        for (int i = 0; i < inctlInfos.size(); i++) {
            try {
                ...
                log.debug("inctlInfo: " + inctlInfo.getFullFileName());
                ...
                impFileInfo.setCheckAcqMemId(checkAcqMemId);
                // handle impFileInfo
                handleImpFileInfo();
            }
            catch (Exception ignore) {
    ...
    private void insertImportDataLogInfo(Connection conn, DataLineInfo lineInfo, String rcode) throws Exception 
	{
