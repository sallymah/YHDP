 ...
 * author:
 */
public class ExpPTXN extends AbstractExpFile {
    private static Logger log = Logger.getLogger(ExpPTXN.class);
    private static final String SPRING_PATH = "config" + File.separator + "batch" + File.separator +
            "expfiles" + File.separator + "ExpPTXN" + File.separator + "spring.xml";
    private static final String FILE_FN = "PTXN";
    private static final String FILE_APPEND = "PTXN";
    ...
    public ExpFileSetting makeExpFileSetting() {
        ...
        for (int i = 0; i < bankIds.size(); i++) {
            ...
            sb.append("Select a.CARD_NO, a.EXPIRY_DATE, a.TXN_DATE, a.TXN_TIME, a.TXN_AMT, a.ACQ_MEM_ID,");
            sb.append(" a.MERCH_ID, a.LMS_INVOICE_NO, a.P_CODE, a.TXN_SRC from TB_TRANS a, TB_MB_CARD_BAL_REQ b");
            sb.append(" Where a.CARD_NO = b.CARD_NO");
            sb.append(" and a.EXPIRY_DATE = b.EXPIRY_DATE");
            sb.append(" and a.TXN_DATE >= b.EXP_DATE");
            sb.append(" and a.P_CODE='").append("7647").append("'");
            sb.append(" and a.ONLINE_FLAG='F'");
            sb.append(" and a.CUT_DATE='").append(batchDate).append("'");
            sb.append(" and b.EXP_DATE !='").append("00000000").append("'");
            sb.append(" and a.CARD_NO in (select CARD_NO from TB_CARD where BANK_ID='").append(bankId).append("')");
    ...
    public String outputBeforeFile() {    	
        ...
        header.append(StringUtils.rightPad("", 111, ' '));
    ...
    { 
    	...
    	if(memInfo == null) {
    		throw new Exception("acq mem id is not exist");
    	}
    	
    	TbMerchInfo merchInfo = new TbMerchMgr(conn).querySingle(merchId);
    	if(merchInfo == null) {
    		throw new Exception("merch id is not exist");
    	}
...
//        sb.append(Layer2Util.getMaxString(StringUtils.rightPad(merchInfo.getMerchLocName(), 20, ' '),encoding,20));
        ...
        log.info("transInfo: " + dataInfo.toString());
       /* transInfo.setImpDate(impDate);
        transInfo.setCbReasonCode(cbReasonCode);
        transInfo.setStatus(status);*/
	...
	{		
		BankUtil.simulateTrans(conn, dataInfo, batchDate, "5999", "8207");
    	
    	//addMbProblemTxn(dataInfo);
 	}
	...
	public static ExpPTXN getInstance() {
        ApplicationContext apContext = new FileSystemXmlApplicationContext(SPRING_PATH);
        ExpPTXN instance = (ExpPTXN) apContext.getBean("expPTXN");
        return instance;
    }
